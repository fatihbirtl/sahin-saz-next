...
import { FooterLogoTypes } from "@/types"
...
interface FooterLogoProps {
  data: FooterLogoTypes | null
}

export const FooterLogo: React.FC<FooterLogoProps> = ({ data }) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      if (data) {
        await axios.patch(`/api/logo/footer-logo`, values)
      } else {
        await axios.post(`/api/logo/footer-logo`, values)
      }
      router.refresh()
  ...
  return (
    <div>
      <h4 className="text-xl font-semibold tracking-tight mb-4">Footer Logo</h4>
