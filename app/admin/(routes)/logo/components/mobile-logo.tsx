...
import { MobileLogoTypes } from "@/types"
...
interface MobileLogoProps {
  data: MobileLogoTypes | null
}

export const MobileLogo: React.FC<MobileLogoProps> = ({ data }) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      if (data) {
        await axios.patch(`/api/logo/mobile-logo`, values)
      } else {
        await axios.post(`/api/logo/mobile-logo`, values)
      }
      router.refresh()
  ...
  return (
    <div>
      <h4 className="text-xl font-semibold tracking-tight mb-4">Mobile Logo</h4>
